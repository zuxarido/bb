...
function CakeryPage() {
  ...
  return (
    <div className="min-h-screen bg-muted text-foreground selection:bg-bakebook-blue selection:text-background">
      ...
      {/* Hero Section */}
      <section className="mx-auto w-full max-w-[1800px] px-6 pt-40 pb-20 md:px-12 md:pt-48 md:pb-32 lg:pt-56 lg:pb-40 text-center">
        <h1 className="display-caps text-[13vw] sm:text-[10vw] md:text-[8vw] lg:text-[140px] leading-[0.8] tracking-tighter mx-auto max-w-7xl animate-in fade-in slide-in-from-bottom-12 duration-1000 ease-out fill-mode-both">
          CITYWIDE<br />DELIVERY.
        </h1>
        <p className="mt-10 md:mt-14 mx-auto max-w-lg text-lg md:text-xl font-light leading-relaxed text-foreground/70 animate-in fade-in slide-in-from-bottom-8 duration-1000 ease-out fill-mode-both delay-150">
          Dry goods, fresh bakes, and custom cakes. Packed with care and shipped across Delhi.
        </p>
      </section>

      {/* Product Grid - Soft, Airy, Borderless */}
      <section className="mx-auto max-w-[1600px] px-6 pb-40 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-24 gap-x-12 lg:gap-x-20">
            ...
            >
              <div className="relative aspect-[4/5] w-full overflow-hidden bg-background/50 rounded-2xl mb-8 shadow-sm transition-shadow duration-500 hover:shadow-xl cursor-pointer">
                ...
                <div className="absolute inset-x-0 bottom-0 p-6 translate-y-full opacity-0 transition-all duration-500 ease-out group-hover:translate-y-0 group-hover:opacity-100 flex justify-center">
                  <button 
                    ...
                    className="flex items-center gap-2 bg-background/95 backdrop-blur-md text-foreground px-8 py-4 editorial-label rounded-full border border-border/50 shadow-lg hover:bg-bakebook-blue hover:text-background hover:border-bakebook-blue transition-all duration-300"
                  >
                    Add to Bag
              ...
              <div className="flex flex-col flex-1 px-2 text-center md:text-left">
                <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-4 mb-4">
                  ...
                  <span className="editorial-label text-foreground bg-background px-4 py-1.5 rounded-full shadow-sm shrink-0">
                    ₹{product.price}
                  </span>
                </div>
                ...
                <button 
                  onClick={() => addToCart(product)}
                  className="mt-8 md:hidden w-full bg-bakebook-blue/10 text-bakebook-blue py-4 editorial-label rounded-full hover:bg-bakebook-blue hover:text-background transition-colors"
                >
                  Add to Bag
                </button>
              </div>
          ...
          >
            <div className="relative aspect-[4/5] w-full overflow-hidden bg-background rounded-2xl mb-8 flex flex-col items-center justify-center p-10 text-center shadow-sm transition-all duration-500 hover:shadow-xl hover:bg-bakebook-blue group-hover:text-background">
              <div className="h-20 w-20 rounded-full border-2 border-dashed border-muted-foreground/30 flex items-center justify-center mb-8 group-hover:border-background/30 group-hover:scale-110 transition-all duration-500">
                <Plus className="h-8 w-8 text-muted-foreground group-hover:text-background transition-colors duration-500" />
              </div>
              <h3 className="font-display text-4xl tracking-tight leading-[1.1] mb-6">
                Bespoke<br />Cakes
              </h3>
              <p className="text-base md:text-lg font-light leading-relaxed text-muted-foreground group-hover:text-background/80 transition-colors duration-500 max-w-[280px]">
            ...
            <div className="flex flex-col flex-1 px-2">
              <button 
                ...
                className="w-full bg-transparent border border-foreground/20 text-foreground py-4 editorial-label rounded-full hover:bg-bakebook-blue hover:text-background hover:border-bakebook-blue transition-colors duration-300"
              >
                Inquire on WhatsApp
              </button>
            </div>
